...
const scoreElbig= document.querySelector('#scoreElbig')
class Player{
		...
		projectiles.forEach((projectile,projectileindex)=>{
			...
			if (dist - enemy.radius - projectile.radius < 1) {
				...
				for (let i = 0; i <enemy.radius*2; i++) {
					particles.push(
						new Particle(projectile.x,projectile.y,Math.random()*2,enemy.color,{x:(Math.random()-0.5)*(Math.random()*8),y:(Math.random()-0.5)*(Math.random()*8)
